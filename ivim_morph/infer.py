...
def configure_solver_default_args():
    import argparse
    import numpy as np
    ...
    args.path_to_case = None
    return args


def load_case(path_to_data, path_to_seg=None, device='cuda'):
    import os
    import torch
    ...
    if path_to_seg is None:
        return data, None

    seg = torch.load(
        os.path.join(path_to_seg),
        map_location=torch.device(device)
        )
...
def infer_from_data(signal, seg, bvalues, device='cuda'):
    from .src.IVIM_morph import solver_IVIM_Morph

    args = configure_solver_default_args()
    args.bvalues = bvalues

    solver = solver_IVIM_Morph(args = args, device=device)
    warpdata, deformation_field, ivim_params = solver.fit((signal, seg), return_wraped_seg=False)
    D, f, Dp = ivim_params
    return Dp, D, f  # arrange the output so that it's the same as super ivim dc
